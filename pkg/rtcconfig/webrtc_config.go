...
import (
	...
	"github.com/ss14/mediatransportutil/pkg/transport"
)
...
func NewWebRTCConfig(rtcConf *RTCConfig, development bool) (*WebRTCConfig, error) {
	...
	// 		if len(ips) == 0 {
	// 			logger.Infow("no external IPs found, using 172.171.131.186 and 10.224.0.11 for NAT1To1Ips", "ip", rtcConf.NodeIP)
	// 			s.SetNAT1To1IPs([]string{"172.171.131.186", "10.224.0.11"}, webrtc.ICECandidateTypeHost)
	// 		} else {
	...
	if len(localIPs) == 2 {
		loIP0 := localIPs[0]
		// loIP1 := localIPs[1]
		nat1to1IPs = append(nat1to1IPs, fmt.Sprintf("%s/%s", "10.224.0.11", loIP0))
